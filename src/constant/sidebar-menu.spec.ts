...
describe('menuItems', () => {
  ...
  test('Campaign item should be integrated and correctly configured', () => {
    const campaignItem = menuItems.find((item) => item.id === 'campaign');
    expect(campaignItem).toBeDefined();
    if (campaignItem) {
      expect(campaignItem.name).toBe('CAMPAIGN');
